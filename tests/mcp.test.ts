...
describe('MCP server', () => {
  ...
  it('goal tools: add_goal, log_progress, goal_progress, and add_todo with goal', async () => {
    ...
    const detail = await call<{ todos: { open: number }; checkins: unknown[]; open_todos: unknown[]; done_todos: unknown[] }>(client, 'goal_progress', { goal: 'reading' })
    ...
    expect(detail.data.done_todos).toEqual([])
