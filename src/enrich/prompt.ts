...
export function buildSystemPrompt(): string {
  return [
    ...
    '[규모 size] 이 일을 끝내는 데 드는 시간의 크기. 1=하루 미만(연락·구매·심부름·짧은 서류) 2=1~2일(신청·예약·짧은 문서) 3=3~7일(보고서·리서치·발표 자료) 4=2주(기획·설계·긴 문서) 5=2주 이상(프로젝트급). 판단이 안 되면 null.',
    '[준비 기간 lead_days] 보통은 규모에서 정해지므로(1→1, 2→2, 3→5, 4→10, 5→14) null로 두고 서버에 맡긴다. 제목에 "N일 전부터"처럼 명시된 경우에만 그 값을 넣는다.',
